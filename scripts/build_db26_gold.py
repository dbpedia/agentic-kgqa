#!/usr/bin/env python3
"""Pre-compute and cache DB26 gold query results against the evaluation endpoint.

Executes all 50 gold SPARQL queries ONCE against the DBpedia evaluation
...
results instead of re-executing 50 gold queries fresh each time, so any
...
Output: data/db26_gold_final.json
"""
...
QUESTIONS_FILE  = _REPO_ROOT / "benchmark" / "questions_db26.yml"
OUTPUT_FILE     = _REPO_ROOT / "data" / "db26_gold_final.json"
...
def main():
    print("=" * 70)
    print("Building DB26 gold cache")
    ...
    print(f"\n{'='*70}")
    print(f"Done. {len(results)} questions processed, {errors} errors.")
    if timeouts:
